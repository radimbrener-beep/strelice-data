...
.legend{display:flex;flex-wrap:wrap;gap:8px 10px;font-size:12px;color:var(--muted);margin:10px 0}
.legend span{display:inline-flex;align-items:center;gap:6px;cursor:default}
.legend .legi{cursor:pointer;border-radius:7px;padding:2px 7px;transition:.14s;user-select:none;border:1px solid transparent}
.legend .legi:hover{background:var(--inset);border-color:var(--line)}
.legend .legi.off{opacity:.38}
.legend .legclr{color:var(--accent);font-weight:600}
...
function stacked(id,side,keyFn,legId,topN,mode){
  ...
  const sel = LEGSEL[id] || (LEGSEL[id]=new Set());
  const ds=order.map((k,i)=>({label:k,borderRadius:3,backgroundColor:PAL[i%PAL.length],
    hidden: sel.size>0 && !sel.has(k),
    data:YRS.map((y,j)=>{
    let v=(k=='Ostatní')
  ...
  leg.innerHTML = order.map((k,i)=>{
    const off = sel.size>0 && !sel.has(k);
    return `<span class="legi${off?' off':''}" data-k="${k.replace(/"/g,'&quot;')}" title="Klikni pro filtr kategorie">`+
           `<i class="sw" style="background:${PAL[i%PAL.length]}"></i>${k}</span>`;
  }).join('') + (sel.size?` <span class="legi legclr" data-k="__all">&#10005; zrušit filtr</span>`:'');
  leg.querySelectorAll('.legi').forEach(el=>el.onclick=()=>{
    const k=el.dataset.k;
    if(k=='__all') sel.clear(); else (sel.has(k)?sel.delete(k):sel.add(k));
    stacked(id,side,keyFn,legId,topN,mode);
  });
...
const LEGSEL={};   // filtr kategorií v legendě (per graf): množina vybraných kategorií
